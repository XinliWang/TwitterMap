var interval = 30000;
var map, heatmap;
var getTweetURL = window.location.href+ "api/get/twitter";
var crawlTweetURL = window.location.href+ "api/stop/twitter";


var twitter, musicTwitter, newsTwitter, jobTwitter, movieTwitter, 
	androidTwitter, amazonTwitter, gameTwitter, halloweenTwitter;

function addDataIntoMap(getTweetURL) {
	$.getJSON(getTweetURL, function(data) {
		console.log(data);
...
function initDataIntoMap(getTweetURL) {
	$.getJSON(getTweetURL, {crawl:"crawl"},function(data) {
		console.log(data);
		$.each(data['result'], function(index, row) {
			var category = row["category"];
			if (category === "music") {
...
window.onbeforeunload=function(){
	$.get(crawlTweetURL,{"shutdown":"shutdown"});
}
